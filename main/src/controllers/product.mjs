...
//         console.log("_id ", res.slug, " updated (" + j + ")");
    ...
    importproducts1: function (req, res, next) {
        let pp = path.join(__dirname, "./", "hs.json");
        let cccc = 0;
        console.log("pp", pp)
        let start = parseInt(req.params.offset),
            end = start + parseInt(req.params.limit);
        // console.log();

        // console.log(typeof parsedBody);
        let x = _.range(start, end);
        // console.log('x', x);


        fs.readFile(pp, 'utf8', function (err, data) {
            ...
            if (parsedBody)
                _.map(x, function (key) {
                    if (parsedBody[key]) {
                        ...
                        let slug = (parsedBody[key].slug + key) || (convertToSlug(parsedBody[key].title) + cccc + key);
    ...
    importproducts: function (req, res, next) {
        setInterval(() => {
            ...
            Product.findOne({
                ...
                _.forEach((parsedBody.data.images ? parsedBody.data.images : []), async (c, cx) => {
                    // console.log('images[', cx, ']', c);
                    let mainUrl = encodeURI(c);
                    let filename =
                            c.split('/').pop(),
                    ...
                    fstream.on("close", function () {
                        ...
                        Media.create({
                            ...
                            if (err) {
                                ...
                                Product.findByIdAndUpdate(parsedBody._id, {
                                    $push: {
                                        photos: media.url
                                    },
                                    $set: {
                                    ...
                                    console.log('updating product with image...', parsedBody.data.images.length, product.photos.length);
                                    if (err || !product) {
                                        console.log({
                                            err: err,
                                            success: false,
                                    ...
                                    console.log('img /key ', ' created', product._id);
                                    if (product.photos.length == parsedBody.data.images.length)
    ...
    importproducts30: async function (req, res, next) {
        let pp = path.join(__dirname, "./", "hs.json");
        let filePath = path.join(__dirname, './', 'hsnew.json');
        ...
        await fs.readFile(pp, 'utf8', async function (err, data) {
            // console.log("error",err);
            let parsedBody = JSON.parse(data);
            if (parsedBody)
                await _.forEach(x, async function (key) {
                    await new Promise(resolve => setTimeout(resolve, 5000))
                    console.log('parsedBody[key]', parsedBody[key].id)
                    cccc++;
                    if (parsedBody[key].id)
                        await axios({
                            url: "http://gamiha.co/api/shop/product/item?id=" + parsedBody[key].id, //your url
                            method: 'GET',
                        }).then(async response => {
                            console.log("creating #", key)
                            if (response && response.data && response.data.data)
                                await arr.push(response.data.data);
                        }).catch(err => {
                            console.log("err");
                        });
                    console.log("arr.length:", arr.length, "x.length", x.length)
                    if (arr.length == x.length) {
                        console.log("arr", arr);
                        try {
                            fs.promises.writeFile(filePath, JSON.stringify(arr), "utf8");
    ...
    importproducts40: async function (req, res, next) {
        let pp = path.join(__dirname, "./", "hs.json");
        let filePath = path.join(__dirname, './', 'hsnew.json');
    ...
    importproducts3: function (req, res, next) {
        setInterval(() => {
            console.log("start set2...");
            let productt = {};
            Product.findOne({
                "data.set": "set2"
            }, function (err, parsedBody) {
...
//                let mainUrl = encodeURI(c);
                ...
                axios({
                    url: "http://gamiha.co/api/shop/product/item?id=" + parsedBody.data.id, //your url
                    method: 'GET'
                }).then(response => {
                    // console.log('getting mainUrl', mainUrl);
                    let ddd = response.data.data;
                    ddd['set'] = "set3";
                    console.log("ddd", ddd);
                    Product.findByIdAndUpdate(parsedBody._id, {
                        $set: {
