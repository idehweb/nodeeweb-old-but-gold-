...
var self = ( {
                                    ...
                                    if (child.children) {
                                        await _.forEach(child.children, async function (ch, key3) {
                                            ...
                                            await Discount.create({
                                                ...
                                                slug: ch.slug,
                                                parent: discount2._id
                                            }, async function (err, discount3) {
                                                if (err || !discount3) {
                                                    await console.log('err', err);
                                                }
    ...
    all: function (req, res, next) {
        if (req.query.f) {
            ...
            Discount.find(search, function (err, discounts) {
                // console.log('err', err);
                ...
                Discount.countDocuments({}, function (err, count) {
    ...
    allSXml: async function () {
        ...
        return new Promise(async function (resolve, reject) {
            ...
            await Discount.find(search, async function (err, discounts) {
                if (err || !discounts) {
                    return await ([]);
                }
                // await console.log("allSXml1", "allSXml1")
                let cd = new Date();
                await _.forEach(discounts, async (c) => {
                    await XTL.push({
                        url: '/discount/' + c._id + '/' + c.name['fa'],
                        lastMod: cd,
                        changeFreq: 'daily'
                    });
                });
    ...
    s: function (req, res, next) {
        ...
        Discount.find(search, function (err, discounts) {
            ...
            Discount.countDocuments({}, function (err, count) {
                ...
                if (req.params._id)
                    Discount.findById(req.params._id, function (err, mainCat) {
                        // console.log('here');
                        // if (discounts && discounts.length >= 0) {
                        //   if (mainCat) {
                        //     mainCat.back = true;
                        //     mainCat.name = mainCat['name'][req.headers.lan];
                        //     discounts[discounts.length] = mainCat;
                        //   }
                        // }
                        res.json(discounts.reverse());
                        return 0;
                    }).lean();
                else {
                    res.json(discounts.reverse());
    ...
    sidebar: function (req, res, next) {
        ...
        Discount.find(search, function (err, discounts) {
            ...
            Discount.countDocuments({}, function (err, count) {
                ...
                if (req.params.catId)
                    Discount.findById(req.params.catId, function (err, mainCat) {
                        // console.log('here');
                        if (discounts && discounts.length >= 0) {
                            if (mainCat) {
                                mainCat.back = true;
    ...
    exparty: function (req, res, next) {
        ...
        //     }
        // ];
        // Discount.create(s, function (err, discount) {
        //     if (err || !discount) {
        //         res.json({
        //             err: err,
        //             success: false,
    ...
    edit: function (req, res, next) {
        Discount.findByIdAndUpdate(req.params.id, req.body, function (err, discount) {
            if (err || !discount) {
                res.json({
                    success: false,
                    message: 'error!',
