...
const theChip = (props) => {
    ...
    // if (!inS && !single)
    //   return;
    console.log('combinations', combinations, theCombination, actives)
    return (
        [<div className={" mt-5 the-chip row"} key={0}>
            ...
            <div className={" mt-2 the-chip row"} key={1}>
                ...
                <div className={"col-md-4 mt-3-1200"}>{Boolean(theCombination) && <div className={"the-option-price text-center"}>

                    <div className={"gfd"}>
                        <Theprice
                            className={"single single-let " + theCombination.salePrice + " - " + theCombination.price}
    ...
    return (
        <Stack direction="row" className={" mt-2 wrap-stack"} spacing={1}>
            ...
            (opt.values).map((val, j) => {
                return <Chip
                    key={j}
                    icon={(actives && (actives[opt.name] === val.name)) ? <RadioButtonCheckedIcon/> :
